// V2.6.1 foreground asset. All geometry is faceted; no smoothed subdivision.
...
var heroWeapon={limbs:[],rightSegments:[],pull:0,reviewPose:null,drawFingers:[],exports:[]};
makeCityWeapon=function(){
 ...
 function hand(parent,draw){
  ...
  if(!draw){
   for(let i=0;i<4;i++){
    ...
    tube(hand,[[-.040,y,-.013],[.010,y,-.043],[.029,y,-.020],[.017,y,.014]],[rad,rad,rad*.9,rad*.72],i===3?M.leather:M.skin,6);
   ...
   tube(hand,[[-.045,.06,.011],[-.003,.041,-.002]],[.016,.011],M.skin,6);
  }else{
   ...
   for(const [i,y] of [[0,.029],[1,-.018],[2,-.042]]){
    ...
    tube(fg,[[-.023,y,-.018],[-.013,y,-.020],[.002,y,-.004]],[.012,.012,.010],M.skin,6);
   }
 ...
 // Naturally elliptical radius/ulna profile: narrow wrist, dorsal tendons,
 // muscle swell toward the elbow, then a rolled sleeve with irregular folds.
 ...
 function forearm(parent,name,covered=false){
  ...
  loft(arm,[[0,0,0,.030,.026],[-.001,.16,.004,.034,.029],[-.007,.34,.007,.042,.035],[-.011,.60,.003,.057,.043],[-.010,.82,0,.061,.046],[0,1,0,.053,.042]],covered?M.cloth:M.skin,10);
  // Shallow anatomical planes and a small old scar, integrated on the silhouette.
  if(!covered){loft(arm,[[-.018,.18,.018,.003,.005],[-.024,.38,.029,.006,.004],[-.033,.58,.032,.006,.003]],M.skinShade,4);}
 ...
 const leftFore=forearm(left,'LEFT_FOREARM'),leftSleeve=sleeve(left,'LEFT_SLEEVE');heroWeapon.segmentPose(leftFore,wrist,elbow);heroWeapon.segmentPose(leftSleeve,elbow,shoulder);
 ...
 batch(weaponBow,new Set(heroWeapon.limbs.map(l=>l.mesh)));batch(left);batch(drawHand);heroWeapon.rightSegments.forEach(o=>batch(o));batch(weaponArrow);
 // drawFingers were baked into the distinct draw pose; release uses the hand pivot.
 heroWeapon.exports=[['la-recurve-v261',weaponBow],['la-grip-arm-v261',left],['la-draw-arm-v261',right],['la-arrow-v261',weaponArrow]];
...
updateWeapon=function(dt){
 ...
 weaponRelease=Math.max(0,weaponRelease-dt);const release=pose==='release'?.18:weaponRelease,pull=heroWeapon.pull*.34;
 weaponRoot.scale.setScalar(portrait?.48:.76);weaponRoot.position.set(portrait?.12:.27,portrait?-.53:-.68,-1.10);weaponRoot.rotation.set(0,.69,-.14);
 if(!reducedMotion&&!pose){weaponRoot.position.y+=isMoving?Math.sin(cityClock*8)*.004:Math.sin(cityClock*1.6)*.001;weaponRoot.rotation.z+=release*.06;}
 ...
 drawHand.position.copy(nock).add(new THREE.Vector3(follow*.28,0,follow));drawHand.rotation.z=-follow*.8;
 const wrist=drawHand.localToWorld(new THREE.Vector3(-.069,-.071,.019));heroWeapon.right.worldToLocal(wrist);
 const elbow=new THREE.Vector3(.19,.50,.58+pull*.40),shoulder=new THREE.Vector3(.51,.20,.87);
